...
ACTIVE_GUIDE_ID = "recipe-pattern-drops"
ACTIVE_FILENAME = "gear-pattern-drops-ah-price-guide.html"
REDIRECTS = {
    "utility-recipe-drops-ah-price-guide.html": "gear-pattern-drops-ah-price-guide.html#ah-category=recipe-utility",
    "consumable-misc-recipe-drops-ah-price-guide.html": "gear-pattern-drops-ah-price-guide.html#ah-category=recipe-consumables",
}
...
EXPECTED_AUDITED_FINGERPRINT = (
    "5d1c4060ebb20b05621372cda90d2572b4526d8190e5a17e451e1b14dcab5d1d"
)
...
def main() -> int:
    ...
    for entry in index["items"]:
        ...
        if entry.get("guideId") != ACTIVE_GUIDE_ID:
            continue
        if filename != ACTIVE_FILENAME:
            fail(f"Recipe-drop row points at an inactive route: {filename}")
        ...
        counts[filename] += 1
        audited_rows.append(f'{entry["name"]}|{item_id}')

    if dict(counts) != {ACTIVE_FILENAME: 90}:
    ...
    for filename, target in REDIRECTS.items():
        ...
        if f'content="0; url={target}"' not in source:
            fail(f"{filename}: compatibility redirect target changed")
        if f'href="{target}"' not in source:
            fail(f"{filename}: compatibility link target changed")
        if 'location.hash.startsWith("#ah-item=")' not in source:
            fail(f"{filename}: legacy item deep links are not preserved")
