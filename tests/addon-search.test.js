...
assert.ok(addons.every((addon) => Array.isArray(addon.compatibility.notes) && addon.compatibility.notes.length > 0));
...
assert.equal(chatter.compatibility.importantNotes.length, 1);
assert.match(chatter.compatibility.importantNotes[0], /Use guildnotes/);
...
assert.doesNotMatch(auctioneer.generalSetup.join("\n"), /AucAdvanced\.lua/);
...
assert.equal(auctioneer.generalSetupGuides.length, 1);
assert.equal(auctioneer.generalSetupGuides[0].title, "Already have Auctioneer? Protect your scanned data first");
assert.match(auctioneer.generalSetupGuides[0].intro, /market scans, profiles, and BeanCounter transaction history/);
assert.match(auctioneer.generalSetupGuides[0].steps.map((step) => step.instruction).join("\n"), /AucAdvanced\.lua.*BeanCounter\.lua/);
assert.match(auctioneer.generalSetupGuides[0].steps.map((step) => step.instruction).join("\n"), /dated backup folder outside the WoW installation/);
assert.match(auctioneer.generalSetupGuides[0].steps.map((step) => step.instruction).join("\n"), /Run \/reload and check the data again/);
assert.match(auctioneer.generalSetupGuides[0].steps.map((step) => step.instruction).join("\n"), /restore the complete addon-folder backup/);
...
assert.equal(atlasLoot.compatibility.importantNotes.length, 1);
assert.match(atlasLoot.compatibility.importantNotes[0], /same Hellscream package/);
