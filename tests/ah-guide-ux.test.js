...
assert.equal(searchIndex.itemCount, 4088);
assert.equal(searchIndex.vendorRecommendationCount, 486);
assert.equal(new Set(searchIndex.items.map((item) => item.name)).size, 3847);
...
assert.equal(counts.collectibles, 127);
