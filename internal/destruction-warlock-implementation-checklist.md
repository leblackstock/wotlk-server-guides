# Destruction Warlock Implementation Checklist
...
- [x] Verify 0/19/52 talent point total and calculator URL
- [x] Verify spell, glyph, item, source, raid size, and difficulty data
- [x] Extend assets/warlock-tooltips.js with verified IDs
...
- [x] Run node tools/audit-fresh-80-guide.mjs templates/spec-guide/destruction-warlock.config.json
- [x] Run node --check assets/destruction-warlock.js
- [x] Run node --check assets/warlock-tooltips.js
- [x] Run node tools/audit-playbook-ability-icons.mjs templates/spec-guide/destruction-warlock.config.json after the regular icon-density audit
...
- [x] Add all verified entities to data/warlock-entities.json
- [x] Rebuild assets/warlock-tooltips.js with `node tools/build-wowhead-tooltips.mjs data/warlock-entities.json assets/warlock-tooltips.js`
- [x] Confirm registered phrases link in ordinary prose without manual wrappers
- [ ] Confirm Wowhead's WotLK tooltip engine loads and mouseovers work — engine, script response, and 38 registered links verified; visible hover popup not reproduced in local Chrome QA
...
- [x] Confirm every ability/action chip in every spec playbook card renders its own verified WoW icon
- [x] Run `node tools/audit-spec-guide.mjs templates/spec-guide/destruction-warlock.config.json` during drafting
- [x] Run `node tools/analyze-guide-icon-density.mjs --config templates/spec-guide/destruction-warlock.config.json --policy templates/spec-guide/icon-density-policy.json` while tuning visual density
- [x] Run `node tools/audit-playbook-ability-icons.mjs templates/spec-guide/destruction-warlock.config.json` after the regular icon-density audit
- [x] Run `node tools/audit-spec-guide.mjs templates/spec-guide/destruction-warlock.config.json --release`; this automatically enforces icon density first and playbook ability icons second
